          ...
          <div className="current-wrap">
            {
              this.state.currentList.map((project, index) => {
                return (
                  <div className={`item item-${index % 6 + 1} ${index > 5 ? 'item-right' : ''} ${this.state.scroll ? "previous" : "current"}`}>
                    <div className="rank">{project.id + 1}</div>
                    <div className={`title`}>
